...
class EbookApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(
            frame,
            text="EPUB 工作流與 AI 翻譯分開管理；AI 翻譯不是產出 EPUB 的必要步驟。",
            wraplength=760,
        ...
        workflow = ttk.Frame(notebook, padding=4)
        ai_tab = ttk.Frame(notebook, padding=12)
        notebook.add(workflow, text="EPUB 工作流")
        notebook.add(ai_tab, text="AI 翻譯")

        self._build_workflow_tab(workflow)
        self._build_ai_tab(ai_tab)

        self.status = tk.StringVar(value="請先選擇輸入 Path 或 EPUB")
        ttk.Separator(frame, orient="horizontal").pack(fill="x", pady=(8, 8))
        ttk.Label(frame, textvariable=self.status, relief="sunken", anchor="w", padding=7).pack(fill="x", side="bottom")

    def _build_workflow_tab(self, frame: ttk.Frame) -> None:
        ...
        self._path_row(step1, "EPUB", self.epub_var, self.select_epub)

        step2 = ttk.LabelFrame(frame, text="步驟 2：抽取文字", padding=12)
        ...
        ttk.Label(step2, text="翻譯可在 AI 翻譯頁籤進行，也可以自行使用其他 AI / 翻譯工具。", foreground="gray", wraplength=700).pack(anchor="w", pady=(8, 0))
        ...
        self.validate_btn = ttk.Button(row, text="驗證翻譯", command=self.validate_book, state="disabled")
        ...
        ttk.Label(step3, text="即使沒有使用內置 AI 翻譯，也可以把自己的翻譯 TXT 放回 Translating 後驗證並產出。", foreground="gray", wraplength=700).pack(anchor="w", pady=(8, 0))

    def _build_ai_tab(self, frame: ttk.Frame) -> None:
        ttk.Label(frame, text="AI 翻譯", font=("Segoe UI", 16, "bold")).pack(anchor="w")
        ttk.Label(
            frame,
            text="可選功能：使用 OpenAI-compatible API 將抽取出的 TXT 翻譯成繁體中文。也可以完全不使用此頁。",
            foreground="gray",
            wraplength=700,
        ).pack(anchor="w", pady=(4, 14))

        settings = ttk.LabelFrame(frame, text="API 設定", padding=12)
        settings.pack(fill="x", pady=(0, 12))
        self._api_row(settings, "API Base URL", self.base_url_var)
        self._api_row(settings, "Model", self.model_var)
        ...
        self._api_row(settings, "每批字數", self.max_chars_var)
        ttk.Label(
            settings,
            text="API Key 只存在目前程式執行期間，不會寫入 GitHub 或設定檔；每批會在 paragraph 邊界自動切割。",
            foreground="gray",
            wraplength=700,
        ).pack(anchor="w", padx=(108, 0), pady=(3, 0))

        action = ttk.LabelFrame(frame, text="執行", padding=12)
        action.pack(fill="x")
        self.api_translate_btn = ttk.Button(action, text="開始 AI 翻譯", command=self.api_translate, state="disabled")
        self.api_translate_btn.pack(anchor="w")
        ttk.Label(
            action,
            text="翻譯結果會寫入目前書籍的 Translating 資料夾；完成後回到「EPUB 工作流」執行「驗證翻譯」。",
            foreground="gray",
            wraplength=700,
    ...
    def select_input_path(self):
        ...
        if path:
            ...
            self.status.set(f"輸入 Path：{path}")
            self._load_epubs_from_input()

    def select_working_path(self):
        path = filedialog.askdirectory(title="選擇工作資料夾")
        if path:
            self.working_var.set(path)
    ...
    def _set_epub(self, path: Path):
        self.epub = path
        self.epub_var.set(str(path))
        if not self.working_var.get().strip():
            self.working_var.set(str(path.parent))
        self._refresh_workspace()
        self.book = path.stem
        self.extract_btn.config(state="normal")
        self.status.set(f"已選擇：{self.epub.name}")
    ...
    def extract_book(self):
        if not self.epub or not self.workspace or not self.book:
            return
        if self.workspace.exists():
            if not messagebox.askyesno("重新抽取", f"工作資料夾已存在：\n{self.workspace}\n\n重新抽取會清除其中的 Data。是否繼續？"):
                return
        ...
        try:
            ...
            messagebox.showerror("抽取失敗", self.error_text(exc))
            self.status.set("抽取失敗")

    def open_source(self):
        if self.data_book.exists():
            os.startfile(self.data_book / "text")

    def open_translation(self):
        self.translation_dir.mkdir(parents=True, exist_ok=True)
        os.startfile(self.translation_dir)

    def api_translate(self):
        if not self.data_book.is_dir():
            messagebox.showerror("AI 翻譯失敗", "請先在 EPUB 工作流執行「抽取 TXT」。")
            return
        ...
        try:
            ...
            messagebox.showerror("AI 翻譯失敗", "每批字數必須是整數。")
            return
        if not api_key or not base_url or not model:
            messagebox.showerror("AI 翻譯失敗", "請填寫 API Base URL、Model 及 API Key。")
            return
        try:
            self.api_translate_btn.config(state="disabled")
            self.status.set("AI 翻譯中，請勿關閉程式……")
            ...
            self.status.set(f"AI 翻譯完成：{files} 個 TXT，{chunks} 個批次")
            messagebox.showinfo("AI 翻譯完成", f"已完成 {files} 個 TXT，共 {chunks} 個 API 批次。\n\n請回到「EPUB 工作流」執行「驗證翻譯」。")
        except Exception as exc:
            self.api_translate_btn.config(state="normal")
            messagebox.showerror("AI 翻譯失敗", self.error_text(exc))
            self.status.set("AI 翻譯失敗")

    def validate_book(self):
        try:
            result = validate(self.data_book, self.workspace / "Translating", complete=True)
            if result:
                messagebox.showerror("驗證失敗", "翻譯資料未通過完整驗證。\n\n請檢查 ID、段落數量、空白翻譯及日文殘留。")
                self.status.set("驗證失敗")
                return False
            self.status.set("驗證通過")
            messagebox.showinfo("驗證通過", "所有翻譯 TXT 均通過驗證。")
            return True
        except Exception as exc:
            messagebox.showerror("驗證失敗", self.error_text(exc))
            self.status.set("驗證失敗")
            return False

    def build_book(self):
        if not self.data_book.is_dir():
            messagebox.showerror("產出 EPUB 失敗", "請先執行「抽取 TXT」。")
            return
        try:
            ...
            if translation_files and validate(self.data_book, self.workspace / "Translating", complete=False):
                messagebox.showerror("產出 EPUB 失敗", "翻譯資料未通過檢查。請修正後再產出。")
                self.status.set("翻譯驗證失敗")
                return
            self.build_btn.config(state="disabled")
            self.status.set("正在重建 EPUB……")
            self.update_idletasks()
            ...
            self.build_btn.config(state="normal")
            messagebox.showerror("產出 EPUB 失敗", self.error_text(exc))
            self.status.set("產出 EPUB 失敗")
